...
export function useDownload() {
  ...
  const fillWorkerSlots = useCallback(() => {
    ...
    // Only pick up items the user has actually selected.
  ...
  const startAll = useCallback(() => {
    const store = useQueueStore.getState();
    // Only count items the user has actually selected.
    const hasSelectedQueued = store.items.some(
      (i) => i.status === "queued" && i.selected
    );
    if (!hasSelectedQueued) return;
